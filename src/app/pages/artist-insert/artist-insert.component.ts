...
export class ArtistInsertComponent {
  ...
  onSubmit() {
    ...
    this.dataProvider.postNewArtist(this.newArtista).subscribe(
      (response : any) => {
        console.log(response);
        const verificacionElement = document.getElementById('verificacion');
        if (verificacionElement) {
          if (response.error) {
            verificacionElement.innerHTML = response.error;
          } else {
            verificacionElement.innerHTML = 'Artista creado exitosamente.';
          }
        }
      }
    );
